...
function displayEvents(events) {
    ...
    container.innerHTML = events.map(event => `
        <div class="event-card">
            <img src="${event.image_url || placeholder}" alt="${event.title}" class="card-image"
...
document.getElementById('eventForm').addEventListener('submit', async (e) => {
    ...
    // If file provided, upload first and use returned URL
    const imageFileInput = document.getElementById('image_file');
    if (imageFileInput && imageFileInput.files && imageFileInput.files[0]) {
        const f = new FormData();
        f.append('file', imageFileInput.files[0]);
        const up = await fetch('/api/upload', { method: 'POST', headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }, body: f });
        if (up.ok) {
            const j = await up.json();
            data.image_url = j.url;
        } else {
            const err = await up.json().catch(()=>({message:'Upload failed'}));
            alert(err.message);
            return;
        }
    }
